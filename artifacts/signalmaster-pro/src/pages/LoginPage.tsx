...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api.auth.login(user.trim(), pass);
    ...
    <div className="min-h-screen bg-[#07070d] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        ...
        <div className="mt-6 text-center text-sm text-gray-400">
          Não tem uma conta?{" "}
          <Link href="/register" className="text-white font-medium hover:text-[var(--green)] transition">
            Criar conta grátis
          </Link>
        </div>

        <div className="mt-4 flex items-center justify-center gap-1.5 text-[11px] text-gray-600">
          <ShieldCheck size={11} />
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center text-sm text-gray-500 max-w-sm w-full opacity-60">
        SignalMaster Pro v7 · Plataforma profissional de sinais
      </div>
    </div>
  );
}
